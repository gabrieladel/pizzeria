        ...
        <div class="d-flex flex-wrap justify-content-center" style="gap: 25px;">
            ...
            @foreach ($listado as $item)
                ...
                <div class="card pizza-card"
                    style="width: 16rem; border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); overflow: hidden; position: relative; padding-bottom: 60px; border: none;">

                    <img class="card-img-top"
                        src="{{ $item->imagen ? asset('storage/' . $item->imagen) : asset('images/no-image.png') }}"
                        style="height: 12rem; object-fit: cover;" alt="{{ $item->nombre }}">


                    <div class="card-body">
                        <h5 class="card-title" style="font-weight: bold;">{{ $item->nombre }}</h5>
                        <p class="card-text" style="font-size: 0.85rem; color: #666; height: 40px; overflow: hidden;">
                            {{ $item->descripcion }}</p>
                        ...
                        <form action="{{ route('cart.store') }}" method="POST">
                            ...
                            <div style="position: absolute; bottom: 15px; left: 15px; right: 15px;">
                                <button type="submit" class="btn btn-danger btn-pedir w-100"
                                    style="background-color: rgb(127, 26, 46); border: none; border-radius: 8px; font-weight: bold;">
                                    Pedir Ahora
                                </button>
                            </div>
                        </form>
