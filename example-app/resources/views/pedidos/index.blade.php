                    ...
                    <tbody>
                        @foreach ($pedidos as $item)
                            <tr>
                                ...
                                <td>
                                    <strong>{{ $item->cliente->persona->nombre ?? 'N/A' }}
                                        {{ $item->cliente->persona->apellido ?? '' }}</strong>
                                </td>
                                <td>
                                    ...
                                    <div class="btn-group">
                                        @if ($item->estado != 'entregado')
                                            <button class="btn btn-success btn-sm" data-toggle="modal"
                                                data-target="#modalFinalizar{{ $item->id }}">
                                                <i class="fas fa-check-circle"></i> Facturar
                                            </button>
                                        @endif

                                        <button class="btn btn-warning btn-sm" data-toggle="modal"
                                            data-target="#modalEditar{{ $item->id }}">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('pedidos.finalizar', $item->id) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                <p>Cliente: <strong>{{ $item->cliente->persona->nombre ?? 'N/A' }}</strong>
                                                </p>
                                                ...
                                                <div class="alert alert-info">
                                                    Total con IVA:
                                                    <strong>${{ number_format($item->total * 1.21, 2) }}</strong>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="submit" class="btn btn-success">Confirmar y Generar
                                                    Factura</button>
                                            </div>
                                        </form>
                                    </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header bg-secondary text-white">
                    <h5 class="modal-title" id="modalRegistrarLabel"><i class="fas fa-cart-plus"></i> Registrar Nuevo Pedido
                    </h5>
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select name="cliente_id" class="form-control" required>
                                    ...
                                    @foreach ($clientes as $c)
                                        <option value="{{ $c->id }}">{{ $c->persona->nombre }}
                                            {{ $c->persona->apellido }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 form-group">
                                    ...
                                    <select name="productos[]" class="form-control" required>
                                        ...
                                        @foreach ($productos as $p)
                                            <option value="{{ $p->id }}">{{ $p->nombre }} -
                                                ${{ number_format($p->precio, 2) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <input type="number" name="cantidades[]" class="form-control" placeholder="Cant."
